...
class TaoApp(QWidget):
    ...
    def setup_header_service_menu(self):
        ...
        menu_service_layout = QVBoxLayout()
        menu_service_layout.setContentsMargins(0, 0, 0, 0)

        service_label = self.create_label("СЕРВИС", 40)
        service_label.setStyleSheet(config.gradient.gradient_brown)
        menu_service_layout.addWidget(service_label, alignment=Qt.AlignmentFlag.AlignTop)

        button_load = self.create_button("Загрузить видео")
        menu_service_layout.addWidget(button_load)

        return menu_service_layout
